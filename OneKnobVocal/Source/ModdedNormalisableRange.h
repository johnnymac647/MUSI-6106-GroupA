/*
  ==============================================================================
    ...
    ModdedNormalisableRange.h
    Created: 23 Mar 2022 7:12:43pm
    Author:  yinsile
...
class ModdedNormalisableRange
{
public:
    /** Creates a continuous range that performs a dummy mapping. */
    ModdedNormalisableRange() = default;

    ModdedNormalisableRange(const ModdedNormalisableRange&) = default;
    ModdedNormalisableRange& operator= (const ModdedNormalisableRange&) = default;
    ModdedNormalisableRange(ModdedNormalisableRange&&) = default;
    ModdedNormalisableRange& operator= (ModdedNormalisableRange&&) = default;

    /** Creates a ModdedNormalisableRange with a given range, interval and skew factor. */
    ModdedNormalisableRange(ValueType rangeStart,
        ValueType rangeEnd,
    ...
    /** Creates a ModdedNormalisableRange with a given range, continuous interval, but a dummy skew-factor. */
    ModdedNormalisableRange(ValueType rangeStart,
    ...
    /** Creates a ModdedNormalisableRange with a given range and interval, but a dummy skew-factor. */
    ModdedNormalisableRange(ValueType rangeStart,
        ValueType rangeEnd,
    ...
    /** Creates a ModdedNormalisableRange with a given range, continuous interval, but a dummy skew-factor. */
    ModdedNormalisableRange(juce::Range<ValueType> range) noexcept
        : ModdedNormalisableRange(range.getStart(), range.getEnd())
    {
    }

    /** Creates a ModdedNormalisableRange with a given range and interval, but a dummy skew-factor. */
    ModdedNormalisableRange(juce::Range<ValueType> range, ValueType intervalValue) noexcept
        : ModdedNormalisableRange(range.getStart(), range.getEnd(), intervalValue)
    {
    }
    ...
    /** Creates a ModdedNormalisableRange with a given range and an injective mapping function.

        @param rangeStart           The minimum value in the range.
        @param rangeEnd             The maximum value in the range.
        @param convertFrom0To1Func  A function which uses the current start and end of this ModdedNormalisableRange
                                    and produces a mapped value from a normalised value.
        @param convertTo0To1Func    A function which uses the current start and end of this ModdedNormalisableRange
                                    and produces a normalised value from a mapped value.
        @param snapToLegalValueFunc A function which uses the current start and end of this ModdedNormalisableRange
    ...
    */
    ...
    /** Takes a non-normalised value and snaps it based on either the interval property of
        this ModdedNormalisableRange or the lambda function supplied to the constructor.
    */
    ...
    juce::Range<ValueType> getRange() const noexcept { return { start, end }; }
...
private:
    ...
    {
        auto clampedValue = juce::jlimit(static_cast<ValueType> (0), static_cast<ValueType> (1), value);
